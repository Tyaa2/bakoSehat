...
class DataWargaController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($search) {
            $query->where(function ($q) use ($search) {
                $q->where('nama', 'like', "%$search%")
                    ...
                    ->orWhere('nama_puskesmas', 'like', "%$search%")
        ...
        return view('dinkes.data-warga.index', compact('dataWarga', 'filter', 'search', 'sort', 'direction'));
    }
    ...
    public function approve(Request $request, $id)
    {
        ...
        $alasan = $request->input('alasan_penolakan_dinkes');

        if ($action === 'approve') {
            $warga->status_approve_dinkes = 'approved';
            $warga->alasan_penolakan_dinkes = null;
        } elseif ($action === 'reject') {
            $warga->status_approve_dinkes = 'rejected';
            $warga->alasan_penolakan_dinkes = $alasan;
        } else {
            return redirect()->route('dinkes.data-warga.index')->with('error', 'Aksi tidak dikenali.');
        }

        $warga->save();

        return redirect()->route('dinkes.data-warga.index')->with('success', 'Status berhasil diperbarui.');
    }
}
