...
class DashboardRsudController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = DataWarga::where('created_by', $RsudId);
        ...
        // Statistik ringkasan
        $semuaWarga = DataWarga::where('created_by', $RsudId)->get();
        ...
        return view('rsud.dashboard', compact('dataWarga', 'latestWarga', 'total', 'terverifikasi', 'ditolak', 'menunggu', 'user'));
    }
    public function refreshJson()
    {
        try {
            ...
            $latestWarga = DataWarga::where('created_by', $RsudId)
                ->latest('created_at')
                ->take(4)
                ->get()
                ->map(function ($warga) {
            ...
            Log::error('DashboardRsudController@refreshJson error: ' . $e->getMessage());
