...
<div class="container mx-auto p-4 max-w-lg">
    <h1 class="text-2xl font-bold mb-6">Edit Surat Keterangan Domisili</h1>
    ...
    <form action="{{ route('kelurahan.data-warga.update', $warga->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        @method('PUT')

        <div>
            <label for="nik" class="block font-medium mb-1">NIK Warga</label>
            <input type="text" name="nik" id="nik" value="{{ $warga->nik }}" disabled
        ...
        <div>
            ...
            @if ($warga->surat_keterangan_domisili)
                <p class="mt-2 text-sm text-gray-600">
                    File saat ini: 
                    <a href="{{ Storage::url($warga->surat_keterangan_domisili) }}" target="_blank" class="text-blue-600 underline">
                        Lihat Surat Domisili
                    </a>
                </p>
            @endif
        </div>
