<x-app-layout>
        ...
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="relative space-y-8 px-6 max-w-8xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="flex justify-between items-center bg-white/80 shadow-lg p-6 rounded-2xl backdrop-blur">
                    <div>
                        <h1 class="text-2xl font-semibold text-gray-800">Selamat Datang, Admin puskesmas
                            {{ $user->name }}!</h1>
                        <p class="text-gray-600 text-sm">Kelola data pendaftar BPJS gratis secara efisien dan real-time.
                        </p>
                    </div>
                    <div class="flex gap-2">
                        <a href="{{ route('puskesmas.data-warga.index') }}"
            ...
            data.warga.forEach(w => {
                const row = `
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-3 capitalize">${w.status || 'pending'}</td>
                        <td class="px-4 py-3">${formatDate(w.tanggal)}</td>
                        <td class="px-4 py-3">
    ...
    function refreshDashboardData() {
        ...
        fetch('/puskesmas/dashboard-puskesmas/refresh-json', {
...
<script>
    ...
    function getStatusBadge(status) {
        if (!status || status === 'pending') {
            return `<span class="relative inline-block items-center px-2 py-1 font-sans text-xs font-bold text-gray-900 uppercase rounded-md select-none whitespace-nowrap bg-gray-500/20">Belum Diterima</span>`;
        } else if (status === 'approved') {
            return `<span class="relative inline-block items-center px-2 py-1 font-sans text-xs font-bold text-green-900 uppercase rounded-md select-none whitespace-nowrap bg-green-500/20">Diterima</span>`;
        } else {
